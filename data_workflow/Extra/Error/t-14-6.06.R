...
# read_xpt(glue("{adam_lib}/adlbhy.xpt"))

test1 <- read_xpt(glue("{adam_lib}/adlbhy.xpt"))

test2 <- adlbhy <- adam_adlbhy 
...
doc <- rtf_doc(ht2, header_rows = 3) %>% titles_and_footnotes_from_df(
  from.file='~/CDISC_pilot_replication/data/titles.xlsx',
...
write_rtf(doc, file='~/Clinical-Tables-in-R-with-gt/14-6.06.rtf')
